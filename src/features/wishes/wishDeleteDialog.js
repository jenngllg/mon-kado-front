 ...
 * onDeleted: () => void | Promise<void>, onUnavailable: (state: WishUnavailable) => void, signal?: AbortSignal}} options Operations and lifetime.
 * @returns {HTMLDialogElement} Native modal; closing also disposes and removes it.
 */
export function createWishDeleteDialog({ wishlistId, wishId, loadWishlist, loadOne, remove, onDeleted, onUnavailable, signal }) {
  ...
  const title = element("h2", "Supprimer un cadeau"); title.id = `wish-delete-${crypto.randomUUID()}`; title.tabIndex = -1; title.setAttribute("autofocus", "");
  const warning = element("p", "Cette action est définitive. Ce cadeau sera retiré de ta liste. Les autres cadeaux seront conservés."); warning.id = `${title.id}-warning`;
  ...
  const confirm = createButton({ label: "Supprimer définitivement", variant: "danger", onClick: () => { void deleteWish(); } });
  ...
  registerComponentCleanup(dialog, () => {
    ...
    title.textContent = "Supprimer un cadeau"; confirm.disabled = true; cancel.disabled = true;
  ...
  function unavailable(state) {
    blocked = true;
    if (state !== "suspended") { terminal = true; current = null; clearDetails(); title.textContent = "Supprimer un cadeau"; }
  ...
  async function read(explicit) {
    ...
    try {
      ...
      title.textContent = `Supprimer définitivement « ${gift.wish.name} » ?`;
  ...
  async function deleteWish() {
    ...
    busy = true; deleting = true; clearFeedback(); sync(); setButtonLoading(confirm, true); status.textContent = "Suppression de ton cadeau…";
    ...
    if (disposed || lifetime.signal.aborted) return;
    completed = true; current = null; clearDetails(); title.textContent = "Cadeau supprimé"; sync(); back.hidden = false;
  ...
  function failure(error, mutation, parent) {
    ...
    if (error instanceof ApiError && (error.statusCode === 412 || error.statusCode === 428 || error.validationErrors.some(item => item.propertyName === "ifMatch"))) {
      blocked = true; show({ title: "Actualisation nécessaire", message: error.statusCode === 412 ?
    ...
    if (mutation && (!(error instanceof ApiError) || error.kind !== "http" || (error.statusCode !== null && error.statusCode >= 500))) {
      blocked = true; detail.push("La suppression de ton cadeau ne peut pas être confirmée. Relis le cadeau avant de réessayer.");
    }
